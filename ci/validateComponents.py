...
def validate_component_file_names_and_content():
    for spec in COMPONENTS.glob("TMFC*/*"):
        if not spec.name.startswith("TMFC") or not spec.name.endswith(".yaml"):
            ...
            if "BDD" in spec.parts or "ComponentRI" in spec.parts or "ComponentConformanceProfile" in spec.parts or "Diagrams" in spec.parts or "ReleaseNotes" in spec.parts:
                continue
